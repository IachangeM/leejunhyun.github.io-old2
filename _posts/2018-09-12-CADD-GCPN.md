...
* [Jin et al][15] : Variational autoencoder(VAE)을 사용했다. 분자구조를 atom들의 작은 클러스터들로 이루어진 junction tree로 나타냈다.
...
* **State Transition Dynamics** : 이전 state에서 다음 state로 넘어가는 확률분포다. Domain-specific rule은 이 단계에서 반영된다. Policy network에서 정해진 action이 기초적인 화학규칙에 위배되는 transition이면 여기서 쳐내지고 state는 변하지 않는다. [Figure 1](#figure1) (d)에서는 둘다 valency check를 통과했기 때문에 환경이 action에 따라 분자의 state를 업데이트 한다. text-based 방법들과는 달리 graph representation을 이용하면 매 스텝마다 (미완성 상태일지라도) valency check가 가능하다. 

* **Reward Design** : **final reward**와 **intermediate reward**가 둘다 사용된다. final reward는 domain-specific reward와 adversarial reward의 합산으로 정의된고, intermediate reward는 스텝마다 validity reward와 adversarial reward로 정의된다.
    * **Final reward** : Domain-specific reward + Adversarial reward
        ...
        * Adversarial reward : Discriminator로부터 산출된 loss로부터 정의된 $-V(\pi_{\theta},D_{\phi})$
    * **Intermediate reward** : Step-wise validity reward + Adversarial reward
    ...
    $\pi_\theta$는 policy network이거, $D_{\phi}$는 policy network와 같은 구조의 discriminator, $x$ 는 input graph, $p_{data}$ 는 final graph와 intermediate graph 둘다에게 정의되는 data distribution이다.
